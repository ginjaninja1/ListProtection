...
namespace ListProtection.Services
{
    ...
    public static class MemberIdentityLogger
    {
        private static readonly (string Name, Func<GroundTruthMember, bool> IsPopulated)[] Fields =
        {
            ...
            ("ProviderIds",              m => m.ProviderIds != null && m.ProviderIds.Count > 0),
            ...
            ("ParentSeriesProviderIds",  m => m.ParentSeriesProviderIds != null && m.ParentSeriesProviderIds.Count > 0),
        };
        ...
        public static void LogIdentity(GroundTruthMember member, ILogger logger, string tag)
        {
            ...
            if (member.ProviderIds != null && member.ProviderIds.Count > 0)
                logger.Info(
                    "{0}   ProviderIds: {1}",
                    tag, string.Join(", ", member.ProviderIds.Keys));

            if (member.ParentSeriesProviderIds != null && member.ParentSeriesProviderIds.Count > 0)
                logger.Info(
                    "{0}   ParentSeriesProviderIds: {1}",
                    tag, string.Join(", ", member.ParentSeriesProviderIds.Keys));
        }
    }
}